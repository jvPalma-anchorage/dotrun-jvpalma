...
# shellcheck disable=SC2155,SC1091,SC2016
...
exitIfNoBranchName() {
  ...
  if [ -z "$(git_current_branch)" ]; then
    log_error "No branch name found. Are you in a detached HEAD state?"
    exit 1
  fi
}
...
is_remote_branch() {
  # $1 = branch name
  # return 0 if origin/<branch> exists
...
checkout_remote() {
  # $1 = branch name
  echo "→ Remote branch found on origin: $1"
  # fetch the single branch ref (keeps it light), then create local tracking branch
  git fetch origin "$1"
  git checkout -b "$1" --track "origin/$1"
}

create_new_branch() {
  # $1 = branch name (already sanitized)
